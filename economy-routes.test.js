...
test('admin gameplay XP is committed once alongside a real spin',async()=>{const data=seed();data['users/buyer'].isAdmin=true;const h=harness(data);assert.equal((await h.invoke('/spin-wheel',{userId:'buyer'})).status,200);assert.equal(h.data()['users/buyer'].townHallXP,3);await h.invoke('/spin-wheel',{userId:'buyer'});assert.equal(h.data()['users/buyer'].townHallXP,3);});

test('completed market purchases award buyer and seller XP once; repeated owners receive no extra trade XP',async()=>{const data=seed();data['users/buyer'].isAdmin=true;data['users/seller'].isAdmin=true;const h=harness(data);assert.equal((await h.invoke('/buy',{itemId:'card',buyerId:'buyer'})).status,200);assert.equal(h.data()['users/buyer'].townHallXP,2);assert.equal(h.data()['users/seller'].townHallXP,3);await h.invoke('/buy',{itemId:'card',buyerId:'buyer'});assert.equal(h.data()['users/buyer'].townHallXP,2);const repeated=seed();repeated['users/buyer'].isAdmin=true;repeated['users/seller'].isAdmin=true;repeated['items/card'].hallBuyers=['buyer'];repeated['items/card'].hallSellers=['seller'];const again=harness(repeated);assert.equal((await again.invoke('/buy',{itemId:'card',buyerId:'buyer'})).status,200);assert.equal(again.data()['users/buyer'].townHallXP,undefined);assert.equal(again.data()['users/seller'].townHallXP,undefined);});
