...
(() => {
 ...
 let travelling=false,guideShown=false;
 ...
 function create(){
  ...
  d.append(viewport,toolbar,footer,inspector);const rotate=el('section','island-rotate');rotate.setAttribute('aria-label','Landscape view');rotate.append(el('span','island-rotate-icon','?'),el('h2','','Turn your phone sideways'),el('p','','Your island has more room to explore in landscape.'));const stay=el('button','village-secondary','Continue in portrait');stay.onclick=()=>rotate.classList.add('dismissed');rotate.append(stay);d.append(rotate);document.body.append(d);d.addEventListener('cancel',e=>{e.preventDefault();if(!inspector.hidden)inspector.hidden=true;});
  ...
  new ResizeObserver(()=>{if(d.open){reset();}}).observe(viewport);
 }
 ...
 async function show(){if(loading)return;if(!enabled)previousStudio=footyStudio.enabled;loading=true;toggle.disabled=true;back.disabled=true;try{await load();window.VillageInteriors.close();document.getElementById('rubyShopDialog')?.close();if(!d)create();if(!enabled)previousStudio=footyStudio.enabled;enabled=true;document.getElementById('islandLoading').hidden=true;saved(true);toggle.textContent='Village mode: on';toggle.setAttribute('aria-pressed','true');back.hidden=true;options();render();if(!d.open)d.showModal();if(!initialized)reset();viewport.focus({preventScroll:true});guide();}catch(e){const panel=document.getElementById('islandLoading');panel.hidden=false;panel.querySelector('p').textContent=e.message;}finally{loading=false;toggle.disabled=false;back.disabled=false;}}
 async function enterBuilding(b){if(!verified||!enabled||!unlocked(b))return;entering=true;
  try{d.close();inspector.hidden=true;selected=null;tiles.forEach(t=>t.classList.remove('selected'));back.hidden=true;
   ...
   if(b.route==='hall'){VillageInteriors.close();back.hidden=false;await window.openTownHall(show);return;}
