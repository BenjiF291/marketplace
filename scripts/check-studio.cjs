...
const hallRecords={'users/ui-preview':{isAdmin:true,username:'Club captain',townHallXP:60,rubyItems:{'relic:crown':1},gems:{bronze:100}},'users/other':{isAdmin:true,username:'Alex'}},hallHandlers={};
...
(async()=>{
 ...
 if(process.env.TOWN_HALL_CHECK_ONLY==='1'){
  await page.evaluate(()=>openTownHall());await page.locator('#townHallRoom').waitFor({state:'visible'});await page.getByRole('button',{name:'Your character',exact:true}).click();await page.getByLabel('coat',{exact:true}).selectOption('#8b6d9f');await page.getByLabel('style',{exact:true}).selectOption('braids');await page.getByLabel('outfit',{exact:true}).selectOption('armour');await page.getByLabel('cape',{exact:true}).selectOption('long');await page.getByLabel('headwear',{exact:true}).selectOption('circlet');await page.screenshot({path:'.ui-tools/town-hall-character.png'});await page.getByRole('button',{name:'Save character',exact:true}).click();await page.locator('.hall-editor').waitFor({state:'detached'});
  ...
  await page.getByRole('button',{name:'Projects & XP',exact:true}).click();await page.getByRole('button',{name:'Contribute 5 Rubies',exact:true}).click();await page.locator('.hall-upgrade-celebration').waitFor();await page.screenshot({path:'.ui-tools/town-hall-upgrade.png'});assert.equal(hallRecords['users/ui-preview'].townHallXP,110);assert.ok((await p2.locator('#hallLevelLabel').textContent()).includes('Level 1'));await page.locator('.hall-editor').getByRole('button',{name:'Close',exact:true}).click();
