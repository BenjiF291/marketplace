...
const fixture=(url)=>{
 if(url==='/admin/village')return {level:0,maxLevel:9,tiers:petTiers,balance:2450,preview:true};
...
(async()=>{
 ...
 if(process.env.VILLAGE_CHECK_ONLY==='1'){
  ...
  await page.getByLabel('Find a building').selectOption('blacksmith');assert.equal(await page.locator('.village-inspector').getByRole('button',{name:'Visit the gem forge'}).count(),1);
  await page.getByLabel('Preview village progression').selectOption('8');assert.equal(await page.locator('.village-building.locked').count(),0);assert.equal(await page.locator('[data-building="forge"] .village-floating-gem').count(),1);await page.getByLabel('Close building details').click();await page.getByLabel('Recenter map').click();await page.screenshot({path:'.ui-tools/village-desktop-grown.png'});
  await page.getByLabel('Find a building').selectOption('market');await page.getByLabel('Close building details').click();await page.locator('[data-building=market]').click();await page.getByRole('button',{name:'Enter building',exact:true}).click();assert.equal(await page.locator('#villageMap').isVisible(),false);assert.equal(await page.locator('body').getAttribute('data-studio-page'),'marketplace');await page.locator('.village-return').click();
  ...
  await page.getByLabel('Find a building').selectOption('forge');await page.screenshot({path:'.ui-tools/village-forge-mobile.png'});await page.getByRole('button',{name:'Enter building',exact:true}).click();assert.equal(await page.locator('body').getAttribute('data-studio-page'),'workshop');await page.locator('.village-return').click();
  await page.getByRole('button',{name:'Exit village mode'}).click();assert.equal(await page.locator('#villageMap').isVisible(),false);assert.equal(await page.locator('.village-return').isVisible(),false);
  ...
  console.log('Village passed: admin visibility, 13 buildings, progression previews, forge evolution, drag, mobile layout, routes, return and exit.');await browser.close();await new Promise(r=>server.close(r));return;
 }
