...
if(process.env.PUBLIC_ISLAND_CHECK_ONLY==='1')user.isAdmin=false;
...
const hallRecords={'users/ui-preview':{isAdmin:true,username:'Club captain',townHallXP:98,rubyItems:{'relic:crown':1},gems:{bronze:100}},'users/other':{isAdmin:true,username:'Alex'}},hallHandlers={};
...
const fixture=(url)=>{
 if(url==='/admin/village')return {level:0,maxLevel:9,forgeLevel:0,tiers:petTiers,balance:2450,isAdmin:user.isAdmin,preview:false};
...
(async()=>{
 ...
 if(process.env.PUBLIC_ISLAND_CHECK_ONLY==='1'){
  ...
  let mobileMapReads=0;await phone.route('https://marketplace-aw8b.onrender.com/**',async route=>{const path=new URL(route.request().url()).pathname;if(path==='/admin/village'&&++mobileMapReads>1){await new Promise(resolve=>setTimeout(resolve,3000));}try{await route.fulfill({json:fixture(path)});}catch{}});
  const mobile=await phone.newPage();await mobile.goto('http://127.0.0.1:4173/index.html');await mobile.locator('#villageMap').waitFor({state:'visible'});assert.equal(await mobile.locator('.island-rotate').count(),0);const beforeTap=await mobile.locator('.village-world').evaluate(n=>n.style.transform);await mobile.locator('[data-building=townhall]').tap();assert.equal(await mobile.locator('.village-world').evaluate(n=>n.style.transform),beforeTap);
  await mobile.setViewportSize({width:844,height:390});assert.equal(await mobile.locator('.island-rotate').isVisible(),false);
  await mobile.getByLabel('Find a building',{exact:true}).selectOption('market');await mobile.getByRole('button',{name:'Enter Marketplace',exact:true}).click();await mobile.getByRole('button',{name:'Return to island',exact:true}).waitFor();for(let i=0;i<3;i++){const returnStarted=Date.now();await mobile.getByRole('button',{name:'Return to island',exact:true}).click();await mobile.waitForFunction(()=>{const d=document.querySelector('#villageMap');return d.open&&!d.classList.contains('village-returning');});assert.ok(Date.now()-returnStarted<2500,'Map return must not wait for delayed refresh');await mobile.waitForTimeout(100);assert.equal(await mobile.locator('.village-world').evaluate(n=>/NaN|Infinity/.test(n.style.transform)),false);await mobile.screenshot({path:'.ui-tools/mobile-return-'+i+'.png'});await mobile.getByLabel('Find a building',{exact:true}).selectOption('market');await mobile.getByRole('button',{name:'Enter Marketplace',exact:true}).click();await mobile.getByRole('button',{name:'Return to island',exact:true}).waitFor();}await phone.close();
 ...
 if(process.env.TOWN_HALL_CHECK_ONLY==='1'){
  await page.evaluate(()=>Island.enter('townhall'));await page.locator('#townHallRoom').waitFor({state:'visible'});await page.getByRole('button',{name:'Your character',exact:true}).click();await page.getByLabel('coat',{exact:true}).selectOption('#8b6d9f');await page.getByLabel('style',{exact:true}).selectOption('braids');await page.getByLabel('outfit',{exact:true}).selectOption('armour');await page.getByLabel('cape',{exact:true}).selectOption('long');await page.getByLabel('headwear',{exact:true}).selectOption('circlet');await page.screenshot({path:'.ui-tools/town-hall-character.png'});await page.getByRole('button',{name:'Save character',exact:true}).click();await page.locator('.hall-editor').waitFor({state:'detached'});
  const second=await browser.newContext({viewport:{width:1100,height:900},serviceWorkers:'block'});await second.addInitScript(()=>{localStorage.setItem('userId','other');localStorage.setItem('username','Alex');localStorage.setItem('sessionToken','preview-only');});await second.route('https://marketplace-aw8b.onrender.com/**',async route=>{if(route.request().url().includes('/admin/town-hall/'))return hallRoute(route);await route.fulfill({json:fixture(new URL(route.request().url()).pathname)});});const p2=await second.newPage();p2.on('pageerror',e=>errors.push(e.message));await p2.goto('http://127.0.0.1:4173/index.html');await p2.evaluate(()=>Island.enter('townhall'));await page.waitForTimeout(1700);assert.equal(await page.locator('.hall-person').count(),2);assert.equal(await p2.locator('.hall-person').count(),2);
  ...
  await page.getByLabel('Decorate shelf 1',{exact:true}).click();await page.getByRole('button',{name:/Ancient Geode Crown/}).click();await page.locator('.hall-editor').waitFor({state:'detached'});await p2.waitForTimeout(1700);assert.equal(await p2.locator('[data-shelf="0"] svg').count(),1);
  await page.getByRole('button',{name:'Activities & XP',exact:true}).click();await page.locator('.hall-upgrade-celebration').waitFor();await page.screenshot({path:'.ui-tools/town-hall-upgrade.png'});assert.equal(hallRecords['users/ui-preview'].townHallXP,101);assert.ok((await p2.locator('#hallLevelLabel').textContent()).includes('Level 1'));await page.locator('.hall-editor').getByRole('button',{name:'Close',exact:true}).click();
  ...
  assert.deepEqual(errors,[]);console.log('Town Hall UI passed: two live sessions, saved character, owned shared decor, private XP upgrades, seats, agreed Skystones match, mobile layout and game handoff.');await second.close();await browser.close();await new Promise(r=>server.close(r));return;
 }
