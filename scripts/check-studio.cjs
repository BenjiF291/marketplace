...
(async()=>{
 ...
 if(process.env.VILLAGE_CHECK_ONLY==='1'){
  ...
  await page.getByLabel('Preview village progression').selectOption('8');assert.equal(await page.locator('.village-building.locked').count(),0);assert.equal(await page.locator('[data-building="forge"] .village-floating-gem').count(),0);await page.getByLabel('Close building details').click();await page.getByLabel('Recenter map').click();await page.screenshot({path:'.ui-tools/village-desktop-grown.png'});
